...
<body style="background-image: url({{ url('img/flowerBg.png') }})"
    class="bg-center bg-cover bg-fixed items-center justify-center font-montserrat">
    ...
    <div class="flex items-center justify-center">
        <div
            class="flex flex-col  bg-[#f6a192] drop-shadow-[0_15px_15px_rgba(0,0,0,0.3)] w-5/6 md:w-2/3 lg:w-1/2 xl:w-1/3 rounded-xl  py-5 px-10 mt-36">
            <div class="text-2xl font-montserrat font-bold mb-4 text-left">Forgot Password</div>
            <div class="text-base font-montserrat mb-2 text-left">A reset link will be sent to your email</div>
            <form method="POST" action="/forgotPassword" class="w-full">
                @csrf
                <input type="email" name="email" placeholder="email"
                    class="bg-gray-50 border mb-2 border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
                <div><button type="submit" class=" bg-[#F5C390] rounded-md w-full p-1 hov">Send reset link</button>
                </div>
            </form>
        </div>
    </div>
